use crate::{
    infrastructure::io::{
        export::{
            csv::export_csv, excel::export_excel, json::export_json, markdown::export_markdown,
            toml::export_toml, yaml::export_yaml,
        },
        import::{csv::import_csv, json::import_json, toml::import_toml, yaml::import_yaml},
    },
    prelude::{
        CSTError, DomErr, Filter, ImportMode, Priority, Progress, Sort, Status, Task, TaskBuilder,
        TaskRepository,
    },
};
...
impl<R: TaskRepository> TaskService<R> {
    ...
    pub fn list_tasks(&self, sort: Sort) -> Result<Vec<Task>, CSTError> {
        self.repository.read_all(sort)
    }

    /// Returns one page of tasks and the total count.
    pub fn paged_tasks(&self, sort: Sort, page: i64) -> Result<(Vec<Task>, i64), CSTError> {
        self.repository.read_paged(sort, page)
    }

    /// Looks up a single task by ID. Returns `None` if not found.
    pub fn find_task_by_id(&self, id: i64) -> Result<Option<Task>, CSTError> {
    ...
    pub fn update_task(
        ...
        let task = TaskBuilder::from_task(&existing)
    ...
    pub fn import(&self, path: &str, mode: &ImportMode) -> Result<(usize, f64), CSTError> {
        self.import_with(path, mode, import_csv)
    }

    /// Imports tasks from a JSON file. Returns `(count, elapsed_secs)`.
    pub fn import_json(&self, path: &str, mode: &ImportMode) -> Result<(usize, f64), CSTError> {
        self.import_with(path, mode, import_json)
    }

    /// Imports tasks from a YAML file. Returns `(count, elapsed_secs)`.
    pub fn import_yaml(&self, path: &str, mode: &ImportMode) -> Result<(usize, f64), CSTError> {
        self.import_with(path, mode, import_yaml)
    }

    /// Imports tasks from a TOML file. Returns `(count, elapsed_secs)`.
    pub fn import_toml(&self, path: &str, mode: &ImportMode) -> Result<(usize, f64), CSTError> {
        self.import_with(path, mode, import_toml)
    }

    /// Returns the number of tasks that would be imported from a CSV file.
    pub fn import_preview(&self, path: &str) -> Result<usize, CSTError> {
        self.preview_with(path, import_csv)
    }

    /// Returns the number of tasks that would be imported from a JSON file.
    pub fn import_json_preview(&self, path: &str) -> Result<usize, CSTError> {
        self.preview_with(path, import_json)
    }

    /// Returns the number of tasks that would be imported from a YAML file.
    pub fn import_yaml_preview(&self, path: &str) -> Result<usize, CSTError> {
        self.preview_with(path, import_yaml)
    }

    /// Returns the number of tasks that would be imported from a TOML file.
    pub fn import_toml_preview(&self, path: &str) -> Result<usize, CSTError> {
        self.preview_with(path, import_toml)
    }

    /// Parses `path` with `parse` and returns the task count without persisting.
    fn preview_with<F>(&self, path: &str, parse: F) -> Result<usize, CSTError>
    where
        F: Fn(&str) -> Result<Vec<TaskBuilder>, CSTError>,
    {
        Ok(parse(path)?.len())
    }
        ...
        F: Fn(&str) -> Result<Vec<TaskBuilder>, CSTError>,
    {
        ...
        if matches!(mode, ImportMode::Restore) {
            self.repository.delete_all()?;
        }
        tasks
            .iter()
    ...
    pub fn export_csv(&self, output: Option<String>) -> Result<(usize, String, f64), CSTError> {
        let tasks = self.repository.read_all(Sort::default())?;
        export_csv(&tasks, output.as_deref())
    }
    ...
    pub fn export_markdown(
        ...
        let tasks = self.repository.read_all(Sort::default())?;
        export_markdown(&tasks, output.as_deref())
    }

    /// Exports all tasks to an Excel workbook. Returns `(count, path, elapsed_secs)`.
    pub fn export_excel(&self, output: Option<String>) -> Result<(usize, String, f64), CSTError> {
        let tasks = self.repository.read_all(Sort::default())?;
        export_excel(&tasks, output.as_deref())
    }

    /// Exports all tasks to JSON. Returns `(count, path, elapsed_secs)`.
    pub fn export_json(&self, output: Option<String>) -> Result<(usize, String, f64), CSTError> {
        let tasks = self.repository.read_all(Sort::default())?;
        export_json(&tasks, output.as_deref())
    }

    /// Exports all tasks to YAML. Returns `(count, path, elapsed_secs)`.
    pub fn export_yaml(&self, output: Option<String>) -> Result<(usize, String, f64), CSTError> {
        let tasks = self.repository.read_all(Sort::default())?;
        export_yaml(&tasks, output.as_deref())
    }

    /// Exports all tasks to TOML. Returns `(count, path, elapsed_secs)`.
    pub fn export_toml(&self, output: Option<String>) -> Result<(usize, String, f64), CSTError> {
        let tasks = self.repository.read_all(Sort::default())?;
        export_toml(&tasks, output.as_deref())
    }

    /// Deletes multiple tasks. Returns `(deleted_ids, missing_ids)`.
    pub fn remove_many(&self, ids: Vec<i64>) -> Result<(Vec<i64>, Vec<i64>), CSTError> {
        self.repository.save_snapshot()?;
    ...
    pub fn done_many(&self, ids: Vec<i64>) -> Result<(Vec<i64>, Vec<i64>), CSTError> {
        self.repository.save_snapshot()?;
    ...
    pub fn update_many(
        ...
        priority: Option<Priority>,
        status: Option<Status>,
    ) -> Result<(Vec<i64>, Vec<i64>), CSTError> {
        self.repository.save_snapshot()?;
