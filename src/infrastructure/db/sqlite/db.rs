use crate::prelude::{
    DEFAULT_PAGE_SIZE, Err, Filter, NewTask, Priority, Repository, Sort, Status, Task, row_to_task,
    {
...
impl SqliteTaskRepository {
    ...
    pub fn new(path: &str) -> Result<Self, Err> {
        let conn = Connection::open(path).map_err(Err::from)?;
        let db = Self { conn };
        db.init_schema()?;
        Ok(db)
    }
    ...
    fn init_schema(&self) -> Result<(), Err> {
        self.conn
    ...
    fn ids_to_json(ids: &[i64]) -> Result<String, Err> {
        serde_json::to_string(ids).map_err(Err::from)
    }
...
impl Repository for SqliteTaskRepository {
    ...
    fn delete_all(&self) -> Result<(), Err> {
        self.conn.execute_batch(DELETE_ALL_TASKS).map_err(Err::from)
    }
